...
static uint32 UTGetNetworkVersion()
{
    ...
    return 0;//BUILT_FROM_CHANGELIST;
}
...
{
    const FString* ReqEntitlementId = &Asset.TagsAndValues.FindTag(FName("Entitlement")).GetValue();//const FString* ReqEntitlementId = Asset.TagsAndValues.Find(TEXT("Entitlement"));
...
void GetAllAssetData(UClass* BaseClass, TArray<FAssetData>& AssetList, bool bRequireEntitlements)
{
    ...
    if (bRequireEntitlements)
    {
        for (int32 i = AssetList.Num() - 1; i >= 0; i--)
        {
            ...
            {
                const FString* ReqAchievement = &AssetList[i].TagsAndValues.FindTag(FName(TEXT("RequiredAchievement"))).GetValue();// Try without FName(TEXT("blah"))
                ...
                {
                    const FString* NeedsItem = &AssetList[i].TagsAndValues.FindTag(FName(TEXT("bRequiresItem"))).GetValue();
...
void GetAllBlueprintAssetData(UClass* BaseClass, TArray<FAssetData>& AssetList, bool bRequireEntitlements)
{
    ...
    else
    {
        ...
        {
            const FString* LoadedParentClass = &LocalAssetList[i].TagsAndValues.FindTag("ParentClass").GetValue();
            if (LoadedParentClass != NULL && !LoadedParentClass->IsEmpty())
            {
                ...
                if (Class != NULL)
                {
                    if (Class->IsChildOf(BaseClass))
                    {
                        ...
                        const FString* GenClassPath = &LocalAssetList[i].TagsAndValues.FindTag("GeneratedClass").GetValue();
        ...
        do
        {
            ...
            for (int32 i = 0; i < LocalAssetList.Num(); i++)
            {
                if (UnloadedBaseClassPaths.Find(*LocalAssetList[i].TagsAndValues.FindTag("ParentClass").GetValue()))
                {
                    AssetList.Add(LocalAssetList[i]);
                    const FString* GenClassPath = &LocalAssetList[i].TagsAndValues.FindTag("GeneratedClass").GetValue();
                    if (GenClassPath != NULL)
    ...
    if (bRequireEntitlements)
    {
        for (int32 i = AssetList.Num() - 1; i >= 0; i--)
        {
            ...
            {
                const FString* ReqAchievement = &AssetList[i].TagsAndValues.FindTag(FName(TEXT("RequiredAchievement"))).GetValue();
                if (ReqAchievement != NULL && !LocallyHasAchievement(**ReqAchievement))
                {
                    UE_LOG(UT, Verbose, TEXT("Don't have achievement for %s"), *AssetList[i].AssetName.ToString());
                    AssetList.RemoveAt(i);
...
void SetTimerUFunc(UObject* Obj, FName FuncName, float Time, bool bLooping)
{
    if (Obj != NULL)
    {
        const UWorld* const World = GEngine->GetWorldFromContextObjectChecked(Obj);
        if (World != NULL)
        {
