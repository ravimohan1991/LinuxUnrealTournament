...
void UParty::InitPIE()
{
    OnPostLoadMap(nullptr);
}

void UParty::OnPostLoadMap(UWorld* sos)
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
...
FString UParty::ToString(EMemberChangedReason InReason)
{
    if(InReason == EMemberChangedReason::Promoted)
        return FString("Promoted");
    if(InReason == EMemberChangedReason::Rejoined)
        return FString("Rejoined");
    if(InReason == EMemberChangedReason::Disconnected)
        return FString("Disconnected");
    return FString("No Clue!");
}

FString UParty::ToString(EMemberExitedReason InReason)
{
    if(InReason == EMemberExitedReason::Left)
        return FString("Left");
    if(InReason == EMemberExitedReason::Kicked)
        return FString("Kicked");
    if(InReason == EMemberExitedReason::Removed)
        return FString("Removed");
    if(InReason == EMemberExitedReason::Unknown)
